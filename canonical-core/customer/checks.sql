...
-- ${schema} (= the domain name) and fails the run if any error-severity
...
SELECT * FROM ${catalog}.${schema}.profile
...
SELECT * FROM ${catalog}.${schema}.address
...
SELECT a.* FROM ${catalog}.${schema}.address a
LEFT JOIN ${catalog}.${schema}.profile p ON a.profile_sk = p.profile_sk
...
SELECT a.* FROM ${catalog}.${schema}.address a
LEFT JOIN (SELECT DISTINCT profile_id FROM ${catalog}.${schema}.profile) p
...
SELECT * FROM ${catalog}.${schema}.contact
...
SELECT c.* FROM ${catalog}.${schema}.contact c
LEFT JOIN (SELECT DISTINCT profile_id FROM ${catalog}.${schema}.profile) p
...
SELECT * FROM ${catalog}.${schema}.consent
...
SELECT c.* FROM ${catalog}.${schema}.consent c
LEFT JOIN (SELECT DISTINCT profile_id FROM ${catalog}.${schema}.profile) p
  ON c.profile_id = p.profile_id
WHERE p.profile_id IS NULL;
...
SELECT * FROM ${catalog}.${schema}.account
...
SELECT a.* FROM ${catalog}.${schema}.account a
LEFT JOIN (SELECT DISTINCT profile_id FROM ${catalog}.${schema}.profile) p
